...
int fitsBits(int x, int n) {
	...
	int compare = (x >> (33 + ~n)) & mask_x; // check for off by 1 err or
...
unsigned float_neg(unsigned uf) {
	...
	// NaN: all bits of exponent (23 - 30) are 1
	int mask_exp = 0xFF << 23;
	int mask_frac = 0x7FFFFF;
	int frac = uf & mask_frac;
        int exp = uf & mask_exp;
        if (exp == 0x7F800000 & exp != 0) {
                return uf;
        }

	int neg_uf;
	int high_bit_mask = 1 << 31;
	int sign_bit = uf & (high_bit_mask);
	if (sign_bit == 0) {
